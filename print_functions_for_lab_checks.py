def check_classifying_images(results_dic):
    ...
    for key, value in results_dic.items():
        # Ensure the dictionary value has at least three elements
        if len(value) < 3:
            print(f"Error: {key} does not have the expected structure. Current value: {value}")
            # Add placeholders to avoid IndexError
            while len(value) < 3:
                value.append(None)

        # Check for matches (value[2] == 1)
        if value[2] == 1:
            n_match += 1
    ...
    print("\nNOT MATCHES:")
    for key, value in results_dic.items():
        # Check for mismatches (value[2] == 0)
        if value[2] == 0:
            n_notmatch += 1
            print(f"{key}: Real: {value[0]}  Classifier: {value[1]}")
...
def check_creating_pet_image_labels(results_dic):
    ...
    for key, value in results_dic.items():
        # Ensure each key maps to a list containing exactly one element
        if len(value) != 1:
